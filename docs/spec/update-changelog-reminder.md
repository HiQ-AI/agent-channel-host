...
3. Conversation 持久化职责提醒间隔：默认 5，范围 0–99；0 只关闭按已完成 turn 数量触发的周期提醒，首轮及职责文本变化后的提醒仍保留。

## 现状与根因

- CLI 没有自更新入口，README 只能让用户手工执行全局 npm 安装。
- 仓库没有 CHANGELOG，发布版本与行为变化缺少面向用户的连续记录。
- 两个 Codex Runtime 都把周期常量硬编码为 5，Conversation 表没有可配置字段，View 和 CLI 也无法修改。
...
- SQLite schema 升级到 v13，为 `conversations` 增加 `responsibility_reminder_interval INTEGER NOT NULL DEFAULT 5 CHECK(0..99)`；旧库迁移后自动取 5。
...
- 数据：新 Conversation 默认 5，0/99 可写，越界拒绝；v12 数据库迁移后为 5。
- Runtime：默认 5 保持原行为，间隔 2 提前提醒，0 不做周期提醒但职责变化仍提醒。
