...
Channel 在群聊、私聊的 `none/selected/all` 订阅策略之外新增独立唤醒词模式开关。最终的独立会话契约以 `independent-wake-session.md` 为准。
...
3. DWS 为匹配消息附加去除唤醒词后的指令；Host 先判断常规订阅，仅在未准入时创建独立唤醒 Conversation。
4. 独立唤醒 Conversation 沿用 inbox 去重与 Worker 调度，但拥有自己的 Codex session。
