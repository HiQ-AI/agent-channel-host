...
用户指出的七个可用性缺口已闭环：Channel 开关现在位于可下钻的独立 Channel 页；群组可按关键词搜索并从候选列表绑定；`→` 下钻、`←` 返回；交互 View 使用 alternate screen，不再把刷新帧写入主终端历史；设置、群搜索和 Instance 向导支持移动文本光标和中间编辑；中文表格按终端显示宽度对齐；退出需二次确认且取消后保留编辑现场。

Round 5 后 `matrix.csv` 共 41 项，`round_5=PASS` 41 项，最终 `status=PASS` 41 项。本轮未连接真实 DWS、未搜索真实群、未启动 Channel owner、未发送消息。
...
- 表格的 natural width、截断和补齐统一使用 grapheme-aware 终端显示宽度；中文、全角字符、常见 emoji 计双列，组合标记不额外占列。96 列 Instance 设置 renderer 对每个 `│` 边界做逐行显示宽度断言。
- reducer 增加独立退出确认态。第一次 `q/Ctrl+C` 不停机；确认层显示 View-owned Host 数量，再次 `q/Ctrl+C/Enter` 才退出，`Esc/←` 取消且不清空导航、编辑或搜索草稿。外部 SIGINT/SIGTERM 仍走即时安全收尾。
...
- npm 0.4.0 pack dry-run PASS，共 72 files，tarball 104.2 kB；
...
- CSV 独立回读：41 cases，41 round-5 PASS，41 final PASS，0 non-PASS。

## 真实 Windows TTY smoke

脚本：`scripts/tui-terminal-smoke.mjs`。

隔离状态根：`D:\baibu-agent\scratchpad\agent-channel-view-sg3h-20260803-110155`。
...
脚本在真实 PowerShell TTY/raw mode 注入合成搜索结果，实际捕获成对 `?1049h/?1049l` 和光标隐藏/恢复序列。字段编辑把 `小小鹏` 通过 Home、Right、中间插入和 End 改为 `小·小鹏` 并原子保存；第一次 `q` 显示“退出确认”，Esc 取消后 View 继续运行，第二次 `q` 后以 Enter 确认退出，最终 transcript 仍以 alternate-screen 恢复序列结尾。

## 独立 CLI 回读

对同一隔离状态根执行实际 `node dist/src/cli.js view --once` 和 `conversation list`：

- `view --once` 独立回读为 2320 UTF-8 bytes，包含三个 Instance；
