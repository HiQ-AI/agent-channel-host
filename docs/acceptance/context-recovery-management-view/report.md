...
Round 5 后的 41 项用例全部 PASS。`agent-channel-host` 0.4.0 已具备：
...
- `npm run verify`：58/58 tests PASS，0.4.0 的 72-file pack dry-run PASS；
- 真实 Codex：`new → resumed`，同一 provider session 前缀，结构化 `silent`；
- 隔离 CLI：两个 instance 的 bare `view --once` 聚合输出和无 `--instance` help PASS；
- 真实 Windows TTY：跨 Instance 总览、Channel/会话下钻、Channel toggle、合成群搜索与绑定、字段光标编辑、TUI 新增、独立全局设置、退出确认/取消、alternate-screen 恢复和退出 PASS；
