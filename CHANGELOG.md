...
- Host 启动时自动为 View 中已登记的群聊和私聊创建或恢复固定 Codex session，并设置桌面端可见名称，格式分别为 `群聊 · <会话名称>` 和 `私聊 · <会话名称>`；该过程不启动 turn 或常驻 Worker。
...
- 群聊和私聊实时订阅之外新增本人消息定时补拉：默认在上一轮完成 5 秒后扫描已启用会话，排除历史结果中带 AI 标记的发送，按消息 ID 去重后直接写入原会话 inbox；扫描水位按会话持久化，失败不推进。
