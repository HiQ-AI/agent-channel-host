...
- 修复 Host 重启补拉在 DWS 实时与历史消息 ID 缺失或变化时可能重复准入的问题；现在优先按消息 ID，并以同会话、同发送者、同发生时刻和完整结构化正文作为精确持久化幂等键，避免已传入 Agent 的消息再次投递。
- 修复活动 turn 只领取实时订阅消息、导致本人定时补拉消息长期停留在 `admitted` 且被后续实时消息越过的问题；实时与本人补拉消息现在按 Conversation sequence 统一 steer，启动历史和任务续接仍保留到下一 turn。
- 修复 `directs=all` 在 Host 离线期间首次出现的普通私聊无法被启动补拉发现的问题；启动时使用一次最长 24 小时、最多 20 页或 1000 条的全局历史查询，并复用现有准入与 inbox 幂等路径。
- 修复单个钉钉系统类私聊历史查询失败导致整个 Host `HOST_FATAL` 的问题；补拉现在按 Conversation 隔离失败、记录可观测错误并继续其他目标。
