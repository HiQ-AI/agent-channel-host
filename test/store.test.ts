...
test('唤醒词模式只标记本人严格前缀消息，是否兜底准入由 Host 决定', () => {
  ...
  assert.equal(applyWakeWordSubscription(config, event, '孙鹏')?.wakeWordInstruction, '检查发布状态');
  assert.equal(applyWakeWordSubscription(config, { ...event, senderName: '同事甲' }, '孙鹏')?.wakeWordInstruction, undefined);
  const aiEvent = { ...event, source: { ...event.source, aiTag: true } };
  assert.equal(isCurrentUserHumanMessage(aiEvent, '孙鹏'), false);
  assert.equal(applyWakeWordSubscription(config, aiEvent, '孙鹏')?.wakeWordInstruction, undefined);
  assert.equal(applyWakeWordSubscription(config, { ...event, content: ' 小小鹏：检查' }, '孙鹏')?.wakeWordInstruction, undefined);
  assert.equal(applyWakeWordSubscription(config, { ...event, content: '小小鹏： ' }, '孙鹏')?.wakeWordInstruction, undefined);
  const direct = { ...event, kind: 'direct' as const, content: '小小鹏, 处理私聊事项' };
  assert.equal(applyWakeWordSubscription(config, direct, '孙鹏')?.wakeWordInstruction, '处理私聊事项');
  config.channel.wakeWordEnabled = false;
  assert.equal(applyWakeWordSubscription(config, event, '孙鹏')?.wakeWordInstruction, undefined);
});
...
test('本人消息补拉持久化独立扫描水位并固定回退两秒', () => {
  ...
  const fallback = new Date('2030-08-04T02:00:00.000Z');
  assert.equal(store.selfChatPollStart('dingtalk', 'default', fallback).toISOString(), fallback.toISOString());
  store.completeSelfChatPoll('dingtalk', 'default', new Date('2030-08-04T02:00:10.000Z'));
  assert.equal(store.selfChatPollStart('dingtalk', 'default', fallback).toISOString(), '2030-08-04T02:00:08.000Z');
  store.failSelfChatPoll('dingtalk', 'default', fallback, '自聊查询失败');
  assert.equal(store.selfChatPollStart('dingtalk', 'default', fallback).toISOString(), '2030-08-04T02:00:08.000Z');
  store.close();
});
...
test('本人身份取当前 profile，补拉保留本人的所有历史消息', () => {
  ...
  assert.deepEqual(currentProfileIdentity({
    currentProfile: 'corp:user',
    profiles: [{ profile: 'corp:user', userId: 'self-user', userName: '本人' }],
  }), { userId: 'self-user', userName: '本人' });
  const store = new Store(':memory:');
  const conversation = store.addConversation({
...
test('资料页自聊使用当前 userId 查询，并投影为本人私聊事件', async () => {
  const config = defaultConfig('self-chat', '.', '小小鹏');
  const calls: string[][] = [];
  const events = await fetchSelfChatHistory(
    config,
    'self-user-id',
    '孙鹏',
    new Date('2026-08-10T20:00:00+08:00'),
    new Date('2026-08-10T20:30:00+08:00'),
    async (_config, args) => {
      calls.push(args);
      return { success: true, result: { hasMore: false, messages: [{
        openMessageId: 'self-message', senderName: '孙鹏', senderOpenDingTalkId: 'self-open-id',
        createTime: '2026-08-10 20:20:48', content: '小小鹏：检查状态',
      }] } };
    },
  );
  assert.deepEqual(calls[0]?.slice(0, 7), [
    'chat', 'message', 'list', '--user', 'self-user-id', '--time', '2026-08-10 20:00:00',
  ]);
  assert.equal(events[0]?.kind, 'direct');
  assert.equal(events[0]?.conversationExternalId, 'self-open-id');
  assert.equal(events[0]?.conversationTitle, '孙鹏');
  assert.equal(events[0]?.messageId, 'self-message');
});
...
test('启动 reconciliation 只恢复可重试 inbox，不执行遗留 outbox', () => {
  const store = new Store(':memory:');
  ...
  assert.equal(store.getOutbox(exhausted.id)?.state, 'sending');
  store.close();
});
...
test('DWS 只启动配置允许的共享 consumer，全部 none 时不启动事件流', () => {
  ...
  config.channel.wakeWordEnabled = true;
  assert.deepEqual(subscribedEventKeys(config), [GROUP_EVENT, DIRECT_EVENT]);
});
...
test('DWS 其他业务错误保持脱敏', async () => {
  ...
  assert.match(structured.message, /reason=business_error/);
});
...
test('v1 会话迁移后补 onboarding 和每类生命周期默认值', () => {
  ...
  try {
    ...
    assert.equal((migrated.db.prepare('PRAGMA user_version').get() as { user_version: number }).user_version, 16);
...
test('本机介入邮箱按 requestId 幂等、按 Conversation 领取并避免未知结果重放', () => {
  const store = new Store(':memory:');
  const conversation = store.addConversation({
    kind: 'direct', externalId: 'mailbox-user', title: '邮箱私聊', responsibility: '', mode: 'shadow',
  });
  const input = {
    requestId: 'request-1', conversationId: conversation.id,
    expectedThreadId: 'thread-1', expectedTurnId: 'turn-1', instruction: '调整方向',
    expiresAt: new Date('2026-08-14T00:01:00.000Z'),
  };
  try {
    assert.equal(store.submitIntervention(input).created, true);
    assert.equal(store.submitIntervention({ ...input, expiresAt: new Date('2026-08-14T00:02:00.000Z') }).created, false);
    assert.throws(
      () => store.submitIntervention({ ...input, instruction: '另一条内容' }),
      /requestId 幂等冲突/,
    );
    assert.equal(store.claimIntervention('other-conversation', 'worker-1', new Date('2026-08-14T00:00:00Z')), null);
    const claimed = store.claimIntervention(conversation.id, 'worker-1', new Date('2026-08-14T00:00:00Z'), 1_000)!;
    assert.equal(claimed.state, 'claimed');
    assert.equal(store.claimIntervention(conversation.id, 'worker-2', new Date('2026-08-14T00:00:02Z')), null);
    assert.deepEqual(
      { state: store.getIntervention('request-1')?.state, code: store.getIntervention('request-1')?.resultCode },
      { state: 'rejected', code: 'outcome_unknown' },
    );

    store.submitIntervention({
      ...input, requestId: 'request-expired', expiresAt: new Date('2026-08-14T00:00:01Z'),
    });
